...
- (id) init	{
	...
	if (self != nil)	{
		...
		//	disable app nap
		self.activity = [[NSProcessInfo processInfo] beginActivityWithOptions:NSActivityUserInitiated reason:@"OSCQuery Server"];
		...
		//NSLog(@"\t\toscIn is %@",oscIn);
		
		//	populate the OSC address space with a series of OSC nodes!
		//	commented out because we're loading from a file right now...
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
	...
	//	check to see if there's a "SampleDocument.json" in ~/Documents/OSCQuery Server
	NSFileManager		*fm = [NSFileManager defaultManager];
	NSString			*tmpDirPath = [@"~/Documents/OSCQuery Server" stringByExpandingTildeInPath];
...
- (IBAction) sliderUsed:(id)sender	{
	...
	[server
		sendOSCPacketData:[packet payload]
...
- (void) _loadLastFile	{
	...
	if (tmpString == nil)	{
		NSString			*samplePath = [@"~/Documents/OSCQuery Server/SampleDocument.json" stringByExpandingTildeInPath];
...
- (void) _loadJSONFile:(NSString *)fullPath	{
	...
	NSString			*bjName = [NSString stringWithFormat:@"%@ OSCQuery Server",fileName];
...
- (BOOL) server:(VVOSCQueryServer *)s wantsToListenTo:(NSString *)address	{
	...
	//	find the delegate object that will take the OSCMessage from the node and send it to the query server
	for (QueryServerNodeDelegate *delegate in delegates)	{
		if ([[delegate address] isEqualToString:address])	{
			[listenNode addDelegate:delegate];
			break;
		}
	}
...
- (void) server:(VVOSCQueryServer *)s wantsToIgnore:(NSString *)address	{
	...
	//	remove from the OSCNode any delegates that are "QueryServerNodeDelegate" instances that match this query server
	NSMutableIndexSet	*indexesToRemove = nil;
	int					tmpIndex = 0;
	MutNRLockArray		*nodeDelegateArray = [listenNode delegateArray];
	[nodeDelegateArray wrlock];
	for (ObjectHolder *anObj in [nodeDelegateArray array])	{
		...
		if (delegate != nil && [delegate isKindOfClass:[QueryServerNodeDelegate class]])	{
			id				delegateQueryServer = [(QueryServerNodeDelegate*)delegate queryServer];
	...
	if (indexesToRemove != nil)
		[[nodeDelegateArray array] removeObjectsAtIndexes:indexesToRemove];
	[nodeDelegateArray unlock];
	
	/*
	//	remove the QueryServerNodeDelegate instance from 'delegates'
	indexesToRemove = nil;
	tmpIndex = 0;
	for (QueryServerNodeDelegate *tmpDelegate in delegates)	{
	...
	if (indexesToRemove != nil)
		[delegates removeObjectsAtIndexes:indexesToRemove];
	*/
}
