...
this framework aims to provide classes to make it easier to implement this OSC query proposal in software that already has an existing OSC implementation- it does so in a simple, generic and what i hope is a library-agnostic fashion.

this framework does *NOT* send or receive OSC data, nor does it provide classes to let you create an OSC address space- this is all functionality that has already been implemented in a variety of other OSC libraries.  instead, this framework tries to give you the tools you need to add support for the OSC query protocol to whatever OSC implementation you're already working with.
